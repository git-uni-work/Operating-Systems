...
class CCargo
{
  public:
    ...
    CCargo( int fee, int weight, int volume ) : m_Fee ( fee ), m_Weight ( weight ), m_Volume ( volume ){}
...
class CShip
{
  public:
    ...
    CShip( std::string destination, int maxWeight, int maxVolume )
    : m_Destination ( move ( destination ) ), m_MaxWeight ( maxWeight ), m_MaxVolume ( maxVolume ){}
    //---------------------------------------------------------------------------------------------
...
class CCustomer
{
  public:
    ...
    virtual void Quote ( const std::string & destination, std::vector<CCargo> & cargo ) = 0;
};
